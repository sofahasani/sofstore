...
<body>
  ...
  <header class="header">
    <div class="header-left">
      ...
      <div class="dropdown" id="dropdownMenu">
        ...
        <a href="#" onclick="event.preventDefault(); if(!isAuthenticated) { showLoginModal(); } else { window.location.href='{{ route('orders.index') }}'; }"><span>📦</span> Pesanan Saya</a>
        <a href="#" onclick="event.preventDefault(); if(!isAuthenticated) { showLoginModal(); } else { window.location.href='{{ route('wishlist.index') }}'; }"><span>❤️</span> Wishlist</a>
        ...
        <a href="{{ route('help.index') }}"><span>💬</span> Bantuan</a>
        @if(auth()->check() && auth()->user()->role === 'admin')
        <a href="{{ route('admin.dashboard') }}"><span>⚡</span> Admin Panel</a>
        @endif
        @if(auth()->check())
        <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><span>🚪</span> Logout</a>
        @else
        <a href="{{ route('login') }}"><span>🔐</span> Masuk</a>
        <a href="{{ route('register') }}"><span>✨</span> Daftar</a>
        @endif
      </div>
  ...
  <div style="background: linear-gradient(135deg, #fff3cd 0%, #ffe7a0 100%); border-left: 4px solid #ff6b35; margin: 16px 16px 0 16px; padding: 12px 16px; border-radius: 12px; display: flex; align-items: center; gap: 12px; box-shadow: 0 2px 8px rgba(255,107,53,0.15); animation: slideDown 0.5s ease;">
    ...
    <button onclick="window.location.href='{{ route('profile') }}'" style="background: #ff6b35; color: white; border: none; padding: 8px 16px; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; white-space: nowrap; transition: all 0.2s;" onmouseover="this.style.background='#ff5722'" onmouseout="this.style.background='#ff6b35'">
      Update Now
    </button>
  </div>
  <style>
        ...
        @forelse($products as $product)
          <a href="{{ route('products.show', $product->id) }}" class="product-card">
            <div class="product-image-container">
              ...
              @if($product->stock)
                <div class="stock-badge">Stok: {{ $product->stock > 999 ? '999+' : $product->stock }}</div>
              @endif
            </div>
            ...
            <div class="product-info">
              ...
              <div class="price-section">
                ...
                @if($hasDiscount)
                  <div class="original-price">Rp{{ number_format($originalPrice, 0, ',', '.') }}</div>
                @endif
              </div>
                ...
                <div class="meta-left">
                  ...
                  @if($totalSold > 0)
                    <div class="sold-count">
                      {{ $totalSold > 1000 ? number_format($totalSold/1000, 1) . 'rb' : $totalSold }} terjual
                    </div>
                  @endif
                </div>
              </div>
  ...
  {{-- Hide floating cart icon karena sudah ada di header --}}
  <style>
